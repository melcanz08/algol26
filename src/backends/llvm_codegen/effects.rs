...
impl<'ctx> IRCodeGen<'ctx> {
    ...
    pub(super) fn emit_print(&self, val: BasicValueEnum<'ctx>, ty: Type) -> Result<()> {
        ...
        let (val, ty) = match ty {
            Type::Borrow(inner) | Type::MutBorrow(inner) | Type::Pointer(inner) => {
                if !val.is_pointer_value() {
                    // The type says this is a reference, but the LLVM
                    // value isn't a pointer. That means the codegen
                    // lost track of the reference at some earlier
                    // step. Failing closed rather than printing the
                    // raw value as if it were the inner type.
                    return Err(CompileError::unsupported_operation(
                        &format!(
                            "print of reference/pointer type expected a pointer value, got {:?}",
                            val
                        ),
                        "llvm",
                    ));
        ...
        let printf_fn = self.module.get_function("printf").ok_or_else(|| {
            // `register_stdlib` runs before any print can occur, so
            // reaching this arm means the module was built without
            // calling into stdlib registration. A silent no-op here
            // would make `print(x)` compile but produce no output.
            CompileError::unsupported_operation(
                "print (printf not registered in the module; register_stdlib was not called)",
                "llvm",
            )
        })?;
