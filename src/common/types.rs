...
impl Type {
    ...
    pub fn ptr() -> Self { Type::Ptr }
    pub fn unknown() -> Self { Type::Unknown }
    pub fn never() -> Self { Type::Never }
    pub fn type_var(name: &str) -> Self { Type::TypeVar(name.to_string()) }
    pub fn generic(name: &str, args: Vec<Type>) -> Self { Type::Generic { name: name.to_string(), args } }
    ...
    pub fn from_str(s: &str) -> Self {
        ...
        match s_lower.as_str() {
            ...
            _ => {
                ...
                if let Some(inner) = s_lower.strip_prefix("list<").and_then(|s| s.strip_suffix('>')) {
                    Type::list(Type::from_str(inner))
                } else if let Some(inner) = s_lower.strip_prefix("option<").and_then(|s| s.strip_suffix('>')) {
                    Type::option(Type::from_str(inner))
                } else if let Some(inner) = s_lower.strip_prefix("channel<").and_then(|s| s.strip_suffix('>')) {
    ...
    pub fn is_primitive(&self) -> bool {
        matches!(self, Type::Int | Type::Float | Type::String | Type::Bool | Type::Void)
    }
    ...
    pub fn is_pointer_like(&self) -> bool {
        matches!(self, Type::Ptr | Type::Pointer(_) | Type::Borrow(_) | Type::MutBorrow(_))
    }
    ...
    pub fn common_supertype(&self, other: &Type) -> Type {
        ...
        match (self, other) {
            ...
            (Type::Ptr, Type::Pointer(t)) | (Type::Pointer(t), Type::Ptr) => Type::Pointer(t.clone()),
            
            // List common element type
            (Type::List(a), Type::List(b)) => {
                Type::list(a.common_supertype(b))
            },
            
            // Option common inner type
            (Type::Option(a), Type::Option(b)) => {
                Type::option(a.common_supertype(b))
            },
...
mod tests {
    ...
    fn test_common_supertype() {
        ...
        assert_eq!(Type::TypeVar("T".to_string()).common_supertype(&Type::Int), Type::Int);
        assert_eq!(Type::Int.common_supertype(&Type::TypeVar("T".to_string())), Type::Int);
    }
    ...
    fn test_display() {
        ...
        assert_eq!(
            Type::Result { ok: Box::new(Type::Int), error: Box::new(Type::String) }.to_string(),
            "Result<Int, String>"
        );
    }
}