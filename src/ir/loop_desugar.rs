...
use crate::frontend::ast::{Expr, FunctionDecl, Stmt, BinOp};
    ...
    for stmt in stmts {
        match stmt {
            Stmt::VarDecl { name, value, type_annotation, mutable, span } => {
                ...
                result.push(Stmt::VarDecl { name, value: desugared_value, type_annotation, mutable, span });
            }
        ...
        if let Expr::List(elements) = &resolved_iterable {
                    ...
                    if !has_complex_cf(&body) && trailing_expr.is_none() {
                        ...
                        result.push(Stmt::For { var, iterable: resolved_iterable, body: desugared_body, trailing_expr });
                    }
                } else {
                    let desugared_body = desugar_stmts(body, env);
                    result.push(Stmt::For { var, iterable: resolved_iterable, body: desugared_body, trailing_expr });
            ...
            Stmt::While { condition, body, trailing_expr } => {
                let desugared_body = desugar_stmts(body, env);
                result.push(Stmt::While { condition, body: desugared_body, trailing_expr });
            }
            ...
            Stmt::Assign { name, value } => {
                ...
                result.push(Stmt::Assign { name, value: desugared });
            }
            Stmt::Print { expr } => {
                result.push(Stmt::Print { expr: desugar_expr(expr, env) });
            }
            other => {
...
fn desugar_expr(expr: Expr, env: &mut HashMap<String, Vec<Expr>>) -> Expr {
    match expr {
        Expr::For { var, iterable, body, trailing_expr, span } => {
            ...
            Expr::For { var, iterable: Box::new(resolved), body: desugared_body, trailing_expr: desugared_trailing, span }
        }
        Expr::While { condition, body, trailing_expr, span } => {
            let desugared_body = desugar_stmts(body, env);
            let desugared_trailing = trailing_expr.map(|te| Box::new(desugar_expr(*te, env)));
            Expr::While { condition, body: desugared_body, trailing_expr: desugared_trailing, span }
        }
        Expr::Block { statements, trailing_expr } => {
            let desugared_stmts = desugar_stmts(statements, env);
            let desugared_trailing = trailing_expr.map(|te| Box::new(desugar_expr(*te, env)));
            Expr::Block { statements: desugared_stmts, trailing_expr: desugared_trailing }
        }
...
fn has_complex_cf(stmts: &[Stmt]) -> bool {
    stmts.iter().any(|s| stmt_has_complex_cf(s))
}
...
fn stmt_has_complex_cf(stmt: &Stmt) -> bool {
    match stmt {
        ...
        | Stmt::RegionBlock { body, .. } | Stmt::UnsafeBlock { body } => body.iter().any(|s| stmt_has_complex_cf(s)),
        Stmt::Parallel { blocks } => blocks.iter().any(|b| b.iter().any(|s| stmt_has_complex_cf(s))),
        _ => false,
    }
}
...
fn expr_has_complex_cf(expr: &Expr) -> bool {
    match expr {
        Expr::Block { statements, trailing_expr } => {
            statements.iter().any(|s| stmt_has_complex_cf(s))
                || trailing_expr.as_ref().map_or(false, |e| expr_has_complex_cf(e))
        }
        Expr::If { then_branch, else_branch, .. } => {
            expr_has_complex_cf(then_branch)
                || else_branch.as_ref().map_or(false, |e| expr_has_complex_cf(e))
        }
        Expr::Match { cases, .. } => cases.iter().any(|c| expr_has_complex_cf(&c.body)),
        Expr::TryCatch { try_branch, catch_branch, .. } => expr_has_complex_cf(try_branch) || expr_has_complex_cf(catch_branch),
        Expr::For { body, trailing_expr, .. } | Expr::While { body, trailing_expr, .. } => {
            body.iter().any(|s| stmt_has_complex_cf(s))
                || trailing_expr.as_ref().map_or(false, |e| expr_has_complex_cf(e))
        }
        _ => false,
    }
                ...
                if let Some(cond_val) = eval_const_expr(&condition) {
                    if cond_val {
                        if let Expr::Block { statements, .. } = then_branch.as_ref() {
                            for s in statements {
                                if matches!(s, Stmt::Break) { return result; }
                                result.push(s.clone());
                            }
                        }
                    ...
                    result.push(Stmt::Expression(Expr::If { condition, then_branch, else_branch }));
                }
            }
            _ => result.push(stmt),
        }
...
fn substitute_var_literal(stmts: &[Stmt], old_name: &str, literal: &Expr) -> Vec<Stmt> {
    stmts.iter().map(|stmt| match stmt {
        ...
        Stmt::Expression(expr) => Stmt::Expression(substitute_expr_literal(expr, old_name, literal)),
        Stmt::VarDecl { name, value, type_annotation, mutable, span } => Stmt::VarDecl {
            name: name.clone(),
            value: substitute_expr_literal(value, old_name, literal),
...
fn substitute_expr_literal(expr: &Expr, old_name: &str, literal: &Expr) -> Expr {
    match expr {
        ...
        Expr::If { condition, then_branch, else_branch } => Expr::If {
            ...
            else_branch: else_branch.as_ref().map(|e| Box::new(substitute_expr_literal(e, old_name, literal))),
        },
        Expr::Block { statements, trailing_expr } => Expr::Block {
            statements: statements.iter().map(|s| match s {
                Stmt::Expression(e) => Stmt::Expression(substitute_expr_literal(e, old_name, literal)),
                _ => s.clone(),
            }).collect(),
            trailing_expr: trailing_expr.as_ref().map(|e| Box::new(substitute_expr_literal(e, old_name, literal))),
        },
        Expr::FunctionCall { name, args, span } => Expr::FunctionCall {
            name: name.clone(),
            args: args.iter().map(|a| substitute_expr_literal(a, old_name, literal)).collect(),
            span: *span,
        },
        Expr::For { var, iterable, body, trailing_expr, span } => {
            if var == old_name {
                Expr::For { var: var.clone(), iterable: iterable.clone(), body: body.clone(), trailing_expr: trailing_expr.clone(), span: *span }
            } else {
                Expr::For {
                    var: var.clone(),
                    iterable: Box::new(substitute_expr_literal(iterable, old_name, literal)),
                    body: body.iter().map(|s| match s {
                        Stmt::Expression(e) => Stmt::Expression(substitute_expr_literal(e, old_name, literal)),
                        _ => s.clone(),
                    }).collect(),
                    trailing_expr: trailing_expr.as_ref().map(|e| Box::new(substitute_expr_literal(e, old_name, literal))),
        ...
        Expr::While { condition, body, trailing_expr, span } => Expr::While {
            condition: Box::new(substitute_expr_literal(condition, old_name, literal)),
            body: body.iter().map(|s| match s {
                Stmt::Expression(e) => Stmt::Expression(substitute_expr_literal(e, old_name, literal)),
                _ => s.clone(),
