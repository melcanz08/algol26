...
impl RegionAllocator {
    ...
    pub fn allocate(&mut self, region_name: &str, size: usize) -> Result<NonNull<u8>, String> {
        ...
        if freed {
            return Err(format!("Region '{}' is freed", region_name));
        }
        if let Some(parent_name) = parent_opt {
            if let Some(p) = self.regions.get(&parent_name) {
                if p.freed {
                    return Err(format!("Parent region '{}' is freed", parent_name));
                }
            }
        }
        ...
        self.memory
            .get_mut(region_name)
            .ok_or_else(|| format!("No memory entry for region '{}'", region_name))?
            .push(block);
    ...
    pub fn deallocate(&mut self, region_name: &str, ptr: NonNull<u8>) -> Result<(), String> {
        let region = self
            ...
            .ok_or_else(|| format!("Region '{}' not found", region_name))?;
        if region.freed {
            return Err(format!("Region '{}' is already freed", region_name));
        }
        let blocks = self
            .memory
            .get_mut(region_name)
            .ok_or_else(|| format!("No memory blocks for region '{}'", region_name))?;
    ...
    pub fn is_valid_ptr(&self, region_name: &str, ptr: NonNull<u8>) -> bool {
        self.memory
            .get(region_name)
            .is_some_and(|blocks| blocks.iter().any(|b| b.ptr == ptr && b.allocated))
    }
    ...
    pub fn get_region_info(&self, name: &str) -> Option<&Region> {
...
mod tests {
    ...
    fn test_create_and_allocate() {
        ...
        a.create_region("test").unwrap();
        let ptr = a.allocate("test", 100).unwrap();
        assert!(a.is_valid_ptr("test", ptr));
    }
    #[test]
    ...
    #[test]
    fn test_allocate_in_freed_region_fails() {
        let mut a = RegionAllocator::new();
        a.create_region("test").unwrap();
        a.free_region("test").unwrap();
        assert!(a.allocate("test", 100).is_err());
    }
    #[test]
    fn test_double_free_prevented() {
        ...
        a.create_region("test").unwrap();
        let ptr = a.allocate("test", 100).unwrap();
        a.deallocate("test", ptr).unwrap();
        assert!(a.deallocate("test", ptr).is_err());
    }
    #[test]
    fn test_child_region_management() {
        let mut a = RegionAllocator::new();
        a.create_region("parent").unwrap();
        a.create_child_region("parent", "child").unwrap();
        a.free_region("parent").unwrap();
