...
fn run_interp(prog: SemanticProgram) -> String {
    ...
    interp.run().unwrap_or_else(|e| format!("interp_err: {:?}", e))
}
...
#[test] fn test_negative_invalid_syntax() {
    let source = "procedure main\n val x := (1 + 2";
    let lexer = Lexer::new(source.to_string()).unwrap();
    let mut parser = Parser::new(lexer.tokens);
    assert!(parser.parse_program().is_err());
}
#[test] fn test_negative_undefined_variable() {
    let source = "procedure main\n print(undefined_var)\n";
    let (_ir, diags) = build_ir(source); assert!(!diags.is_empty());
}
#[test] fn test_negative_type_mismatch() {
    let source = r#"procedure main
    val x := "hello" + 5.0
"#;
    let (_ir, diags) = build_ir(source); assert!(!diags.is_empty());
}
#[test] fn test_stress_nested_control_flow() {
    let source = r#"procedure main
    ...
    print(total)
"#;
    let (_ir, diags) = build_ir(source); assert!(diags.is_empty(), "got {:?}", diags);
}
#[test] fn test_stress_nested_functions() {
...
#[test] fn test_stress_multiple_imports() {
...
"#;
    let lexer = Lexer::new(source.to_string()).unwrap();
    let mut parser = Parser::new(lexer.tokens);
    let prog = parser.parse_program().unwrap(); assert!(!prog.functions.is_empty());
}
...
#[test] fn test_stress_10_level_nested_if_for_defer_break_return() {
    ...
    for i in (0..10).rev() { src.push_str(&format!("{}sum := sum + i{}\n", " ".repeat(i+2), i)); }
    src.push_str(" print(sum)\n");
    let start = Instant::now(); let result = std::panic::catch_unwind(|| build_ir(&src));
    assert!(result.is_ok()); assert!(start.elapsed().as_secs_f64() < 2.0);
}
#[test] fn test_stress_10_level_closure_capture() {
    let mut src = String::new();
    for i in 0..10 {
        src.push_str(&format!("function f{}(x{}: float) -> float\n", i, i));
        if i > 0 { src.push_str(&format!(" return f{}(x{} + 1.0) + x{}\n", i-1, i, i)); }
        else { src.push_str(&format!(" return x{} + 1.0\n", i)); } src.push_str("\n");
    }
    src.push_str("procedure main\n val r := f9(0.0)\n print(r)\n");
    let start = Instant::now(); let (ir, diags) = build_ir(&src);
    assert!(start.elapsed().as_secs_f64() < 2.0); assert!(diags.is_empty(), "{:?}", diags); assert!(!ir.functions.is_empty());
}
#[test] fn test_stress_100_vars_single_scope() {
    let mut src = String::from("procedure main\n");
    for i in 0..100 { src.push_str(&format!(" val v{} := {}.0\n", i, i)); }
    src.push_str(" var sum := 0.0\n");
    for i in 0..100 { src.push_str(&format!(" sum := sum + v{}\n", i)); }
    src.push_str(" print(sum)\n");
    let start = Instant::now(); let (_ir, diags) = build_ir(&src);
    assert!(start.elapsed().as_secs_f64() < 2.0); assert!(diags.is_empty(), "{:?}", diags);
}
#[test] fn test_optimization_preserves_semantics() {
    let (before, after) = run_before_after(r#"procedure main
    val x := 5.0 + 3.0
    val y := x * 2.0
    print(y)
"#);
...
#[test] fn test_optimizer_idempotent() {
    let source = r#"procedure main
    val x := 5.0 + 3.0
    print(x)
"#;
    let (mut ir, _) = build_ir(source); let mut opt = Optimizer::new(); opt.optimize(&mut ir);
    let mut opt2 = Optimizer::new(); opt2.optimize(&mut ir); assert_eq!(opt2.stats.folded_constants, 0);
}
#[test] fn test_optimization_preserves_arithmetic() {
    let (before, after) = run_before_after(r#"procedure main
    val a := 10.0 + 20.0 * 2.0
    val b := (a - 5.0) / 5.0
    val c := b * b
...
#[test] fn test_optimization_preserves_strings() {
    let (before, after) = run_before_after(r#"procedure main
    val greeting := "Hello"
    val name := "World"
...
#[test] fn test_optimization_preserves_lists() {
    let (before, after) = run_before_after(r#"procedure main
    val arr := [1.0, 2.0, 3.0]
    var sum := 0.0
    for x in arr do
...
#[test] fn test_optimization_preserves_borrow_semantics() {
    let source = r#"procedure main
    var x := 5.0
    val r := &x
    print(*r)
"#;
    let (prog, diags) = build_ir(source); assert!(diags.is_empty(), "borrow sample valid: {:?}", diags);
    let borrow_before = prog.functions.iter().flat_map(|f| f.blocks.iter()).flat_map(|b| b.instructions.iter()).filter(|i| format!("{:?}", i).contains("Borrow") || format!("{:?}", i).contains("AddrOf")).count();
    let mut after_prog = prog.clone(); let mut opt = Optimizer::new(); opt.optimize(&mut after_prog);
    let before_out = run_interp(prog); let after_out = run_interp(after_prog.clone());
    assert_eq!(before_out, after_out, "borrow semantics broken");
    if borrow_before > 0 { println!("borrow ops before={}, after={}", borrow_before, after_prog.functions.iter().flat_map(|f| f.blocks.iter()).flat_map(|b| b.instructions.iter()).filter(|i| format!("{:?}", i).contains("Borrow") || format!("{:?}", i).contains("AddrOf")).count()); }
}
#[test] fn test_optimization_diff_interpreter_complex() {
    let (before, after) = run_before_after(r#"function add(a: float, b: float) -> float
    return a + b
procedure main
    val x := 5.0 + 3.0
    val y := x + x
